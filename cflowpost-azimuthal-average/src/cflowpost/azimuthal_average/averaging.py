...
import pandas as pd
import numpy as np
from cflowpost.primitive.datatypes import (CartesianVectorStack,
                                           SymmetricCartesianTensorStack,
                                           )
from cflowpost.primitive import dataparsing
...
def take_azimuthal_running_average(csv_list,
                                   write_intermediate=False,
                                   copy=True):
    ...
    for slice_number, csv_file in enumerate(csv_list):
        ...
        slice_df = dataparsing.csv_to_dataframe(csv_file)
...
def write_intermediate_file(dir_, slice_number, slice_):
    dataparsing.check_create_dir(dir_)
...
def transform_coordinates_to_cylindrical(dataframe):
    ...
    cart_pos_vectors = CartesianVectorStack(cart_pos_array)
...
def transform_velocity_to_cylindrical(dataframe, theta):
    ...
    u_cart_vectors = CartesianVectorStack(vector_array=u_array)
...
def transform_stress_tensor_to_cylindrical(dataframe, theta):
    cart_stress_tensors = SymmetricCartesianTensorStack(
